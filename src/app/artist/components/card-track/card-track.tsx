    ...
    >
      ...
      <div className="track__number">
        {!isHover ? (
          ...
          <button type="button" className="track__play button-play-one">
            <IoPlaySharp />
          </button>
        )}
      </div>
      <button type="button" className={classesCardAdded} onClick={handleAdded}>
        {!isAdded ? <IoAddSharp /> : <IoCheckmarkSharp />}
      </button>
      <div className={classesCardTitle}>
        <span className="title">{track.title}</span>
        {track.featured && (
          <a href={track.featuredUrl} className="feature">
            {track.featured}
          </a>
        )}
      </div>
      {breakpoint && (
        <div className="track__explicit">
          <Tag text={track.explicit} />
        </div>
      )}
      {track.length && <div className="track__length">{track.length}</div>}
      {breakpoint && !track.src && (
        <div className="track__popularity">
          {!track.trend ? <IoTrendingDownSharp /> : <IoTrendingUpSharp />}
        </div>
      )}
      {track.src && <div className="track__plays">{track.plays}</div>}
    </div>
  );
};
